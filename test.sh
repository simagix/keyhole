...
if [ $? != 0 ]; then
    echo "go run keyhole.go --seed mongodb://localhost:30097/KEYHOLEDB"
    exit
fi
...
go run keyhole.go --index mongodb://localhost:30097/KEYHOLEDB
if [ $? != 0 ]; then
    exit
fi
...
go run keyhole.go --schema --collection cars mongodb://localhost:30097/KEYHOLEDB
if [ $? != 0 ]; then
    exit
fi
